...
for i in {-5..29}; do
    ...
    if [[ -f "$output_file" ]]; then
        CN_cation=$(awk '$3 == "Mg" {print $4; exit}' "$output_file")  # First row CN
        C6_cation=$(awk '$3 == "Mg" {print $5; exit}' "$output_file")  # First row CN
	echo "$CN_cation"

        CN_anion=$(awk '$3 == "Zr" {print $4; exit}' "$output_file")  # First row CN
        C6_anion=$(awk '$3 == "Zr" {print $5; exit}' "$output_file")  # First row CN
