...
for i in {-5..29}; do
    ...
    if [[ -f "$output_file" ]]; then
        CN_cation=$(awk '$3 == "Cu" {print $4; exit}' "$output_file")  # First row CN
        C6_cation=$(awk '$3 == "Cu" {print $5; exit}' "$output_file")  # First row CN
	echo "$CN_cation"

        CN_anion=$(awk '$3 == "Pd" {print $4; exit}' "$output_file")  # First row CN
        C6_anion=$(awk '$3 == "Pd" {print $5; exit}' "$output_file")  # First row CN
